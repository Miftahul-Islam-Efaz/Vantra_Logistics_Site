...
export default function Globe({
    ...
    useEffect(() => {
        ...
        let globeOutlineMesh: LineLoop | Mesh | null = null;
        if (showOutline && outlineColor && outlineRgba.a > 0) {
            ...
            if (outlinePoints.length >= 2) {
                ...
                globeOutlineMesh = new LineLoop(outlineGeometry, outlineMaterial);
            }
        }
        ...
        if (showGrid && resolvedGraticuleColor && graticuleRgba.a > 0) {
            ...
            for (let lat = -90; lat <= 90; lat += gridSpacing) {
                ...
                if (positions && positions.length >= 6) {
                    ...
                    if (points.length >= 2) {
                        const gridGeom = new BufferGeometry().setFromPoints(points);
                        const gridLine = new Line(gridGeom, graticuleMaterial);
                        gridLine.renderOrder = 0;
                        graticuleGroup.add(gridLine);
                    }
                }
            }
                        ...
                        const processRing = (ring: number[][]) => {
                            ...
                            if (positions && positions.length >= 6) {
                                ...
                                if (points.length >= 2) {
                                    ...
                                    const line = new LineLoop(geom, outlineMaterial);
                                    line.renderOrder = 0;
